      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-info">
            ...
            <h4 class="card-title"> Find Transfer </h4>
          </div>
            ...
            <div v-if="regiment_id!==''">
              ...
              >
                <template slot="items" slot-scope="props">
                  ...
                  <td>{{ props.item.transferred.name }}</td>
                  <td>{{ props.item.month | monthName }}, {{ props.item.session.name }}</td>
                  <td>{{ props.item.submit_date.slice(0,10) }}</td>
                  <td>
                    <router-link title="Click for view" tag="a" v-bind:to="{ name: 'showTransfer', params: { id: props.item._id } }" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></router-link>
                  </td>
                </template>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        headers: [
          ...
          { text: 'From (Regiment)', value: 'regiment.name' },
          { text: 'Transferred to (Regiment)', value: 'transferred.name' },
          { text: 'Month & Session', value: 'month' },
    ...
    methods: {
      ...
      async findData(){
        if(this.regiment_id!==''){
          const mainData = await ReportService.transfer({regiment_id:this.regiment_id,session_id:this.session_id})

        ...
        <div class="card">
          <div class="card-header card-info">
            <div class="card-btn pull-right">
              ...
              <router-link tag="a" to="/bank-withdraw/create" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> New Withdraw</router-link>
            </div>
            <h4 class="card-title">Find Bank Book Withdraw</h4>
          </div>
            ...
            <div id="printThis">
              <div  v-if="info != '' ">
                <div class="text-center">
                  ...
                  <h5><u> Withdraw from Bank in {{info.session}} </u></h5>
                ...
                <div class="table-responsive">
                  <table id="datatable" class="table table-striped table-bordered">
                    ...
                    <tr v-for="(data,index) in allData" :key="index">
                      <td>{{data.withdraw_date.slice(0,10)}}</td>
                      ...
                      <td>{{data.details}}</td>
                      <td>{{data.amount}}</td>
                      <td class="no-print"> <router-link tag="a" v-bind:to="{ name: 'bankWithdrawShow', params: { id: data._id } }" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></router-link> <router-link tag="a" v-bind:to="{ name: 'bankWithdrawEdit', params: { id: data._id } }" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></router-link> </td>
                    </tr>
                    </tbody>
                  </table>
                </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      async getData () {
        const sessions = await BankWithdrawService.session()
        this.session = sessions.data
      },
      async showData() {
        const showResult = await BankWithdrawService.show(this.form)
        ...
        for(i=0;i<headCount;i++){
          newdata.push({
            withdraw_date: this.allData[i].withdraw_date,

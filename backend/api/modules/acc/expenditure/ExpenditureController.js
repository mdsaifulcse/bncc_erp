...
//Store Data into DB
    async store(req,res,next){
        ...
        req.checkBody('voucher_no', 'Voucher is required').notEmpty()
        let errors = []
        if(req.validationErrors()){
            errors=req.validationErrors()
        }
        let voucher_no = await Expenditure.find({voucher_no:input.voucher_no})
        if(voucher_no.length>0){
            errors.push({msg:'Voucher is already used',param:'voucher_no'})
        }
        if (errors.length>0) {
            let newOne = {}
            errors.forEach(nerr=>{
